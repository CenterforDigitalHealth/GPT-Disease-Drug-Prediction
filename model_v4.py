...
class BinaryChangeHead(nn.Module):
    """
    Binary SHIFT head:
    - class 0: SHIFT label 1
    - class 1: SHIFT labels 2 or 3
    """
...
class MultiHeadOutput(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Binary SHIFT head: label 1 vs labels 2/3
...
class CompositeDelphiConfig:
    ...
    # Binary SHIFT loss options
    # - shift_loss_type: 'dice_focal' | 'focal' | 'ce'
    # - shift_class_weights: class weights for [label1(0), label2or3(1)]
    shift_loss_type: str = 'dice_focal'
    shift_dice_weight: float = 0.5
    shift_ignore_index: int = -1
    shift_maintain_idx: int = 2  # kept for compatibility (unused in v4)
...
class CompositeDelphi(nn.Module):
    ...
    def _compute_loss(self, logits, data, age,
        ...
        if getattr(self.config, 'apply_token_shift', False):
            # shifted labels from raw (1,2,3) -> (2,3,4)
            is_label1 = shift_targets_flat == 2
            is_label2 = shift_targets_flat == 3
            is_label3 = shift_targets_flat == 4
        else:
            is_label1 = shift_targets_flat == 1
            is_label2 = shift_targets_flat == 2
            is_label3 = shift_targets_flat == 3

        shift_valid = is_label1 | is_label2 | is_label3
        shift_logits_flat = shift_logits_flat[shift_valid]
        binary_targets = (is_label2 | is_label3)[shift_valid].long()
