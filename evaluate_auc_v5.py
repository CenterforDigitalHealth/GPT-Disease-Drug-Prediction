...
def evaluate_composite_fields(model, d100k, batch_size=64, device="mps"):
    ...
    if len(all_targets['shift']) > 0:
        ...
        # Optional interpretability metric: rounded class accuracy on valid SHIFT labels.
        shift_pred_rounded = np.rint(np.clip(shift_pred, dec_idx, inc_idx)).astype(np.int64)
        ...
        if len(all_predictions_shift_drug_cond) > 0:
            ...
            shift_drug_pred_rounded = np.rint(np.clip(shift_pred_drug, dec_idx, inc_idx)).astype(np.int64)
