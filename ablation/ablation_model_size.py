...
def _parse_scales(scales_arg: str) -> List[str]:
    scales = [s.strip().lower() for s in scales_arg.split(",") if s.strip()]
    if not scales:
        raise ValueError("No model scales provided.")
    unknown = [s for s in scales if s not in MODEL_SCALES]
    if unknown:
        raise ValueError(f"Unknown scale(s): {unknown}. Available: {sorted(MODEL_SCALES)}")
    return scales
