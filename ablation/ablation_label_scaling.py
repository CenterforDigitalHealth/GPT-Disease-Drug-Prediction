...
def main() -> None:
    p = argparse.ArgumentParser(description='Ablation: label scaling only')
    p.add_argument('--gpu_ids', type=str, default='0')
    p.add_argument('--input_path', type=str, default='../data')
    p.add_argument('--output_root', type=str, default='ablation/label_scaling')
    p.add_argument('--run_name', type=str, default='default')
    p.add_argument('--label_scalings', type=str, default='none,zscore,robust,minmax')
    p.add_argument('--fixed_loss_norm', type=str, default='false', choices=['true', 'false'])
    ...
    for i, scaling in enumerate(scalings, 1):
        trial_name = f't{i:02d}_label_scaling-{scaling}'
        ...
        eval_cmd = [
            ...
            f'--eval_batch_size={args.eval_batch_size}', f'--data_files={args.data_files}',
        ...
        row: Dict[str, object] = {
            'trial_id': i, 'trial_name': trial_name, 'label_scaling': scaling,
