#!/usr/bin/env python3
"""
Ablation runner for label scaling only.

- Training:   train_model
- Evaluation: evaluate_auc

Screens label_scaling values: none, zscore, robust, minmax
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Ablation: label scaling only")
    ...
    parser.add_argument("--output_root", type=str, default="ablation/label_scaling", help="Root output dir")
    parser.add_argument("--run_name", type=str, default="default", help="Run subdirectory name")
    parser.add_argument(
        "--label_scalings",
        type=str,
        default="none,zscore,robust,minmax",
        help="Comma-separated label scaling methods to screen",
    )
    parser.add_argument(
        "--fixed_loss_norm",
        type=str,
    ...
    parser.add_argument(
        "--wandb_run_name",
        type=str,
        default="ablation_label_scaling",
        help="Base run name forwarded to train_model (trial suffix added)",
    )
    args = parser.parse_args()

    # ---- Validate label scalings ----
    scalings = parse_list(args.label_scalings)
    valid = {"none", "zscore", "robust", "minmax"}
    bad = [x for x in scalings if x not in valid]
    if bad:
        raise ValueError(f"Unknown label scaling: {bad}")
    ...
    print("=" * 72, flush=True)
    print("Label scaling ablation started", flush=True)
    print(f"Run root: {run_root}", flush=True)
    print(f"Scalings to screen: {scalings}", flush=True)
    ...
    if existing_rows:
        kept_before_resume = sum(
            1 for tid in trial_rows_by_id if tid < args.resume_from_trial_id
        )
        print(
            f"Existing trials.csv detected: {len(existing_rows)} rows "
            f"(loaded {len(trial_rows_by_id)} rows, "
            f"{kept_before_resume} rows with trial_id < {args.resume_from_trial_id})",
            flush=True,
        )

    # ---- Trial loop ----
    total_trials = len(scalings)

    for trial_idx, scaling in enumerate(scalings, start=1):
        trial_id = trial_idx
        trial_name = f"t{trial_id:02d}_label_scaling-{scaling}"
        ...
        print(
            ...
            f"(id={trial_id:02d}) label_scaling={scaling}",
            flush=True,
        )

        # Initialize row from existing or fresh
    ...
    print("\n" + "=" * 72, flush=True)
    print("Label scaling ablation completed.", flush=True)
