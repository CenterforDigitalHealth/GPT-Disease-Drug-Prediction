...
def _remap_shift_to_change_np(shift_values: np.ndarray, shifted: bool) -> np.ndarray:
    """
    Remap SHIFT labels to binary labels:
    - class 0: label 1
    - class 1: labels 2/3
    """
...
if not shift_class_weights:
    ...
    if master_process:
        print(f"Computed binary class weights (label1 vs label2/3, drug-token subset): {shift_class_weights}")

# WeightedRandomSampler: Patient-level balanced sampling
if master_process:
    print("Computing patient-level sampling weights for label1-vs-label2/3 balancing...")
...
if master_process:
    print(f"  Patients with label2/3 SHIFT events: {minority_patient_count:,} / {len(train_p2i):,}")
