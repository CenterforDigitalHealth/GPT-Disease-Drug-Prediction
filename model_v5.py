...
class CompositeDelphi(nn.Module):
    ...
    def _compute_loss(self, logits, data, age,
        ...
        if getattr(self.config, 'apply_token_shift', False):
            ...
            is_valid_shift = (shift_targets_flat == 2) | (shift_targets_flat == 3) | (shift_targets_flat == 4)
        else:
            # raw labels: 1=Dec, 2=Maint, 3=Inc
            is_valid_shift = (shift_targets_flat == 1) | (shift_targets_flat == 2) | (shift_targets_flat == 3)

        loss_shift = torch.tensor(0.0, device=device)
        loss_change = torch.tensor(0.0, device=device)
        if isinstance(shift_mdn_source, dict):
            shift_pi_logits = shift_mdn_source['pi_logits'].reshape(-1, shift_mdn_source['pi_logits'].size(-1))[shift_pass_tokens]
            shift_mu = shift_mdn_source['mu'].reshape(-1, shift_mdn_source['mu'].size(-1))[shift_pass_tokens]
            shift_log_s = shift_mdn_source['log_s'].reshape(-1, shift_mdn_source['log_s'].size(-1))[shift_pass_tokens]

            shift_pi_logits = shift_pi_logits[is_valid_shift]
            shift_mu = shift_mu[is_valid_shift]
            shift_log_s = shift_log_s[is_valid_shift]
            shift_target = shift_targets_flat[is_valid_shift].float()
            ...
            if shift_target.numel() > 0:
                ...
                loss_shift = torch.clamp(nll_shift, min=0.0, max=20.0).mean()
        else:
            ...
            shift_pred_flat = shift_pred_source.reshape(-1)[shift_pass_tokens][is_valid_shift]
            shift_target = shift_targets_flat[is_valid_shift].float()
            if shift_target.numel() > 0:
                default_shift_scale = 4.0 if bool(getattr(self.config, 'apply_token_shift', False)) else 3.0
                shift_scale = float(getattr(self.config, 'shift_max_value', default_shift_scale))
                if shift_scale <= 0.0:
                    shift_scale = default_shift_scale
                shift_pred_flat = torch.clamp(shift_pred_flat, min=0.0, max=shift_scale)
                loss_shift = F.mse_loss(shift_pred_flat, shift_target) / (max(shift_scale, 1.0) ** 2)
