...
def remap_shift_to_binary_change_torch(shift_values: torch.Tensor, apply_token_shift: bool):
    """
    Remap SHIFT labels to v4 binary labels:
      - class 0: label 1
      - class 1: labels 2 or 3
    Returns:
...
def evaluate_composite_fields(model, d100k, batch_size=64, device="mps"):
    ...
    with torch.no_grad():
        for batch_idx in tqdm(range(num_batches), desc="Evaluating composite fields"):
            ...
            # ============================================================
            # - SHIFT(v4): binary logits (B, T, 2), class0=label1 class1=label2or3
            # - TOTAL: total_head → (B, T) regression output (continuous)
            # ============================================================
            shift_logits = outputs['shift']  # (B, T, 2) for v4
            ...
            # SHIFT(v4): Binary classification (0=label1, 1=label2or3)
    ...
    if len(all_targets['shift']) > 0:
        ...
        results['shift_class_name_map'] = {"0": "Label1", "1": "Label2or3"}
        ...
        if len(all_predictions_shift_drug_cond) > 0:
            ...
            # Force using binary classes: 0=label1, 1=label2or3
