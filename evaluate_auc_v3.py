...
def evaluate_composite_fields(model, d100k, batch_size=64, device="mps"):
    ...
    with torch.no_grad():
        for batch_idx in tqdm(range(num_batches), desc="Evaluating composite fields"):
            ...
            # SHIFT: hierarchical decode if available, else fallback argmax
            if 'shift_change_logits' in outputs and 'shift_direction_logits' in outputs:
                shift_pred = hierarchical_logits_to_shift_labels(
                    outputs['shift_change_logits'],
                    outputs['shift_direction_logits'],
            ...
            if shift_mask.any():
                ...
                if use_drug_conditioning and 'shift_drug_cond' in outputs:
                    if 'shift_change_logits_drug_cond' in outputs and 'shift_direction_logits_drug_cond' in outputs:
                        shift_drug_pred = hierarchical_logits_to_shift_labels(
                            outputs['shift_change_logits_drug_cond'],
                            outputs['shift_direction_logits_drug_cond'],
                            apply_token_shift=eval_apply_token_shift,
                        )
                    else:
    ...
    if len(all_targets['shift']) > 0:
        ...
        # Use fixed class order: dec, maintain, inc
        unique_classes = np.array([dec_idx, maintain_idx, inc_idx])
        ...
        if len(all_predictions_shift_drug_cond) > 0:
            ...
            class_labels = np.array([dec_idx, maintain_idx, inc_idx])
    ...
    if evaluate_composite:
        ...
        if 'shift_accuracy' in composite_metrics:
            ...
            if 'shift_confusion_matrix' in composite_metrics and 'shift_confusion_matrix_classes' in composite_metrics:
                ...
                print(f"\n  Confusion Matrix:")
                print(f"    NOTE: Class ids follow checkpoint tokenization (Dec/Maint/Inc order).")
                print(f"    Predicted →")
