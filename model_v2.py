...
class BinaryChangeHead(nn.Module):
    """
    Binary change classification head:
    - class 0: maintain
    - class 1: changed (decrease or increase)
    """
...
class MultiHeadOutput(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Binary change head (maintain vs changed)
        self.shift_head = BinaryChangeHead(
        ...
        if self.use_drug_conditioning:
            ...
            self.shift_drug_cond_head = BinaryChangeHead(
                n_embd=config.n_embd,
                dropout=float(getattr(config, 'dropout', 0.1)),
            )
...
class CompositeDelphiConfig:
    ...
    # - CHANGE Head: Linear(n_embd, 2) → Softmax + Cross-Entropy (Binary classification)
    ...
    # Binary change head output size is fixed to 2.
    ...
    # No separate auxiliary change head in v2.
    loss_weight_change: float = 0.0
    loss_weight_total: float = 5.0
    loss_weight_time: float = 1.0

    # Binary CHANGE loss options
    # - shift_loss_type: 'dice_focal' | 'focal' | 'ce'
    # - shift_class_weights: class weights for [maintain(0), changed(1)]
    shift_loss_type: str = 'dice_focal'
    shift_dice_weight: float = 0.5
    shift_ignore_index: int = -1
    shift_maintain_idx: int = 2  # kept for compatibility (unused in v2)
    shift_change_weight_max: float = 10.0  # kept for compatibility (unused in v2)
...
class CompositeDelphi(nn.Module):
    ...
    def _compute_loss(self, logits, data, age,
        ...
        # 2. Binary CHANGE Loss (maintain vs changed)
        # label remapping:
        # - raw SHIFT 1/3 -> 1 (changed)
        # - raw SHIFT 2   -> 0 (maintain)
        ...
        if getattr(self.config, 'apply_token_shift', False):
            ...
            is_dec = shift_targets_flat == 2
            is_maint = shift_targets_flat == 3
            is_inc = shift_targets_flat == 4
        else:
            # raw labels: 1=Dec, 2=Maint, 3=Inc
            is_dec = shift_targets_flat == 1
            is_maint = shift_targets_flat == 2
            is_inc = shift_targets_flat == 3

        shift_valid = is_dec | is_maint | is_inc
        shift_logits_flat = shift_logits_flat[shift_valid]
        binary_targets = (is_dec | is_inc)[shift_valid].long()
        ...
        if shift_logits_flat.numel() > 0:
            ...
            weight_t = None
            if isinstance(weights_list, (list, tuple)) and len(weights_list) == 2:
