...
class HierarchicalShiftHead(nn.Module):
    ...
    1) maintain vs changed
    2) (if changed) decrease vs increase
    """
    ...
    def forward(self, x: torch.Tensor):
        change_logits = self.change_head(x)        # maintain(0) vs changed(1)
        direction_logits = self.direction_head(x)  # decrease(0) vs increase(1)
        return change_logits, direction_logits
...
class MultiHeadOutput(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.n_embd = config.n_embd
        self.shift_vocab_size = int(config.shift_vocab_size)
        ...
        self.apply_token_shift = bool(getattr(config, 'apply_token_shift', False))
        if self.apply_token_shift:
            default_dec, default_maintain, default_inc = 2, 3, 4
        else:
            default_dec, default_maintain, default_inc = 1, 2, 3

        dec_cfg = int(getattr(config, 'shift_dec_idx', -1))
        maintain_cfg = int(getattr(config, 'shift_maintain_idx', -1))
        inc_cfg = int(getattr(config, 'shift_inc_idx', -1))
        self.shift_dec_idx = default_dec if dec_cfg < 0 else dec_cfg
        self.shift_maintain_idx = default_maintain if maintain_cfg < 0 else maintain_cfg
        self.shift_inc_idx = default_inc if inc_cfg < 0 else inc_cfg
        if len({self.shift_dec_idx, self.shift_maintain_idx, self.shift_inc_idx}) != 3:
            raise ValueError(
                "SHIFT class indices must be distinct: "
                f"dec={self.shift_dec_idx}, maintain={self.shift_maintain_idx}, inc={self.shift_inc_idx}"
            )
        for idx in (self.shift_dec_idx, self.shift_maintain_idx, self.shift_inc_idx):
            if idx < 0 or idx >= self.shift_vocab_size:
                raise ValueError(
                    f"SHIFT class index {idx} out of range for shift_vocab_size={self.shift_vocab_size}"
                )
    ...
    def _compose_shift_logits(self, change_logits: torch.Tensor, direction_logits: torch.Tensor) -> torch.Tensor:
        """
        Compose final 3-class SHIFT logits from hierarchical outputs.

        Returns:
            shift_logits: (B, T, shift_vocab_size), populated at
              maintain / decrease / increase indices.
        """
        log_change = F.log_softmax(change_logits, dim=-1)     # [maintain, changed]
        log_direction = F.log_softmax(direction_logits, dim=-1)  # [decrease, increase]
        log_maintain = log_change[..., 0]
        log_dec = log_change[..., 1] + log_direction[..., 0]
        log_inc = log_change[..., 1] + log_direction[..., 1]

        shift_logits = torch.full(
            (*change_logits.shape[:-1], self.shift_vocab_size),
            fill_value=-30.0,
            device=change_logits.device,
            dtype=change_logits.dtype,
        )
        shift_logits[..., self.shift_maintain_idx] = log_maintain
        shift_logits[..., self.shift_dec_idx] = log_dec
        shift_logits[..., self.shift_inc_idx] = log_inc
        return shift_logits
    ...
    def forward(self, x, drug_emb=None, drug_token_mask=None):
        ...
        """
        shift_change_logits, shift_direction_logits = self.shift_head(x)
        shift_logits = self._compose_shift_logits(shift_change_logits, shift_direction_logits)
        ...
        output = {
            ...
            'shift_direction_logits': shift_direction_logits,  # (B, T, 2)
        ...
        if self.use_drug_conditioning and drug_emb is not None:
            ...
            shift_change_drug_cond, shift_direction_drug_cond = self.shift_drug_cond_head(shift_modulated)
            shift_drug_cond = self._compose_shift_logits(shift_change_drug_cond, shift_direction_drug_cond)
            ...
            if drug_token_mask is not None:
                ...
                output['shift_direction_logits_drug_cond'] = torch.where(
                    drug_token_mask.unsqueeze(-1),  # (B, T, 1)
                    shift_direction_drug_cond,
                    output['shift_direction_logits']
                )
                ...
                output['shift_direction_logits_drug_cond'] = shift_direction_drug_cond
...
class CompositeDelphiConfig:
    ...
    # - shift_ignore_index: kept for compatibility
    # - shift_class_weights:
    #     * len=2 -> [decrease, increase] for stage-2 direction head
    #     * len=shift_vocab_size -> full SHIFT-index weights (dec/inc indices are used)
    shift_loss_type: str = 'dice_focal'
    shift_dice_weight: float = 0.5
    shift_ignore_index: int = 0
    shift_dec_idx: int = -1
    shift_maintain_idx: int = -1
    shift_inc_idx: int = -1
    shift_loss_drug_only: bool = True
    shift_loss_include_death: bool = False
...
class CompositeDelphi(nn.Module):
    ...
    def _compute_loss(self, logits, data, age,
        ...
        # 2. Hierarchical SHIFT Losses
        # - Stage 1: maintain(0) vs changed(1)
        # - Stage 2: decrease(0) vs increase(1), computed only for changed samples
        change_logits_source = logits.get('shift_change_logits', None)
        direction_logits_source = logits.get('shift_direction_logits', None)
        ...
        if 'shift_direction_logits_drug_cond' in logits and self.config.use_drug_conditioning:
            direction_logits_source = logits['shift_direction_logits_drug_cond']

        shift_targets_all = targets_shift.reshape(-1).long()
        targets_data_all = targets_data.reshape(-1).long()
        ...
        if bool(getattr(self.config, 'shift_loss_drug_only', True)):
            ...
            is_drug_target = (targets_data_all >= drug_token_min) & (targets_data_all <= drug_token_max)
            if not bool(getattr(self.config, 'shift_loss_include_death', False)):
                death_token = 1289 if bool(getattr(self.config, 'apply_token_shift', False)) else 1288
                is_drug_target = is_drug_target & (targets_data_all != death_token)
            shift_pass_tokens = shift_pass_tokens & is_drug_target
        ...
        direction_logits_flat = None
        if direction_logits_source is not None:
            direction_logits_flat = direction_logits_source.reshape(-1, direction_logits_source.size(-1))[shift_pass_tokens]

        if bool(getattr(self.config, 'apply_token_shift', False)):
            default_dec_idx, default_maintain_idx, default_inc_idx = 2, 3, 4
        else:
            default_dec_idx, default_maintain_idx, default_inc_idx = 1, 2, 3
        dec_idx = int(getattr(self.config, 'shift_dec_idx', -1))
        maintain_idx = int(getattr(self.config, 'shift_maintain_idx', -1))
        inc_idx = int(getattr(self.config, 'shift_inc_idx', -1))
        if dec_idx < 0:
            dec_idx = default_dec_idx
        if maintain_idx < 0:
            maintain_idx = default_maintain_idx
        if inc_idx < 0:
            inc_idx = default_inc_idx

        is_dec = shift_targets_flat == dec_idx
        is_maintain = shift_targets_flat == maintain_idx
        is_inc = shift_targets_flat == inc_idx
        shift_valid = is_dec | is_maintain | is_inc
        ...
        if direction_logits_flat is not None:
            direction_logits_flat = direction_logits_flat[shift_valid]

        loss_shift = torch.tensor(0.0, device=device)
        loss_change = torch.tensor(0.0, device=device)
        if shift_targets_flat.numel() > 0:
            valid_is_dec = is_dec[shift_valid]
            valid_is_inc = is_inc[shift_valid]
            change_targets = (valid_is_dec | valid_is_inc).long()

            # Stage-1 change detection loss on all valid SHIFT targets.
            if change_logits_flat is not None and change_logits_flat.numel() > 0:
            ...
            # Stage-2 direction loss only on changed samples.
            changed_mask = change_targets == 1
            if direction_logits_flat is not None and changed_mask.any():
                direction_logits_changed = direction_logits_flat[changed_mask]
                direction_targets = valid_is_inc[changed_mask].long()  # dec=0, inc=1
                ...
                weight_t = None
                if isinstance(weights_list, (list, tuple)):
                    if len(weights_list) == 2:
                        weight_t = torch.tensor(weights_list, device=device, dtype=torch.float32)
                    elif len(weights_list) == int(getattr(self.config, 'shift_vocab_size', 5)):
                        dec_w = float(weights_list[dec_idx]) if 0 <= dec_idx < len(weights_list) else 1.0
                        inc_w = float(weights_list[inc_idx]) if 0 <= inc_idx < len(weights_list) else 1.0
                        weight_t = torch.tensor([dec_w, inc_w], device=device, dtype=torch.float32)

                gamma = float(getattr(self.config, 'shift_focal_gamma', 2.0))
                focal_term = focal_loss_multiclass(
                    direction_logits_changed,
                    direction_targets,
                    gamma=gamma,
                    alpha=weight_t,
                    ignore_index=None,
